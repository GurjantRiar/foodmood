  ...
  return {
    ...
    handleFetch: async function (
      ...
      const dataRequest = await fetch(
        `https://api.edamam.com/search?q=${mainValue}&app_id=08ab47ee&app_key=fa5814a32eb2669676885dff6d983c44&health=${healthValue}&diet=${dietValue}&time=${timeValue}&ingredientsValue=${ingredientsValue}to=30`
      );
    ...
    renderResult: function (data) {
      ...
      //Labels
      const label = data.recipe.dietLabels
        .map((tag) => {
      ...
      //Tags
      const tags = data.recipe.healthLabels
        .map((tag) => {
          return `<span>${tag}</span>`;
        })
        .join("");
      ...
      const ingred = data.recipe.ingredientLines
        .map((ingredient) => {
...
submitButton.addEventListener(
  "click",
  recipeModule.handleSubmit.bind(recipeModule)
);
